...
def img_path_to_encoding(image_path, model):
    ...
    faces = face_recognizer.crop_face(img1)
    if len(faces) == 0:
        print("return none")
        return "None" 
    for (x,y,w,h) in faces:
        img1 = img1[y:y+h, x:x+w]
        print("return bounding box: %s,%s,%s,%s", (x,y,w,h))
        if x < 0 or y < 0:
            print("return none")
            return "None"
...
def img_to_encoding(image, model):
    ...
    elif image.shape[0] < 160 or image.shape[1] < 160:
        image = cv2.resize(image, (160, 160),interpolation=cv2.INTER_CUBIC)
    img = image[...,::-1]

    # img = np.around(np.transpose(img, (2,0,1))/255.0, decimals=20)
    img = np.around(np.transpose(img, (0,1,2))/255.0, decimals=20)
